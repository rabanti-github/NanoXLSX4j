...
public class NumberFormat extends AbstractStyle {
    ...
    /**
     * Sets the raw custom format code in the notation of Excel. <b>The code is not escaped or un-escaped (on workbook
     * loading)</b>
     *
     * @param customFormatCode Custom format code
     * @throws FormatException thrown if the passed value is null or empty
     * @apiNote Currently, there is no auto-escaping applied to custom format strings. For instance, to add a white
     * space, internally it is escaped by a backspace (\ ). To get a valid custom format code, this escaping must be
     * applied manually, according to OOXML specs: Part 1 - Fundamentals And Markup Language Reference, Chapter 18.8.31
     */
    ...
    /**
     * Tries to parse registered format numbers. If the parsing fails, it is assumed that the number is a custom format
     * number (164 or higher) and 'custom' is returned
     *
    ...
    /**
     * Determines whether a defined style format number represents a date (or date and time).<br> Note: Custom number
     * formats (higher than 164), as well as not officially defined numbers (below 164) are currently not considered
     * during the check and will return false
     *
    ...
    /**
     * Determines whether a defined style format number represents a time.<br> Note: Custom number formats (higher than
     * 164), as well as not officially defined numbers (below 164) are currently not considered during the check and
     * will return false
     *
     * @param number Format number to check
    ...
    public static class NumberFormatEvaluation {
        ...
        public enum FormatRange {
            ...
            defined_format,
            /**
             * Custom defined formats from 165 and higher. Although 164 is already custom, it is still defined as enum
             * value
             */
            custom_format,
            /**
            ...
            invalid,
            /**
             * Values between 0 and 164 that are not defined as enum value. This may be caused by changes of the OOXML
             * specifications or Excel versions that have encoded loaded files
             */
            undefined,
        }
