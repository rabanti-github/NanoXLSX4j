...
import ch.rabanti.nanoxlsx4j.registry.PluginLoader;
import ch.rabanti.nanoxlsx4j.utils.LegacyPassword;
...
public class Worksheet {
    ...
    public void addSelectedCells(String range) {
        if (!ParserUtils.isNullOrEmpty(range)) {
    ...
    /**
     * Gets the password wrapper used for sheet protection
     *
     * @return IPassword instance representing the sheet protection password
     * @apiNote If a workbook with password protected worksheets is loaded, only the password hash is loaded.
     * Use {@link IPassword#passwordIsSet()} or {@link IPassword#getPasswordHash()} to check the state
     */
    public IPassword getSheetProtectionPassword() {
        return sheetProtectionPassword;
    }

    /**
     * Sets the password wrapper for sheet protection. This method is used when loading a workbook
     *
     * @param password IPassword instance to set
     */
    void setSheetProtectionPassword(IPassword password) {
    ...
    public void setSheetProtectionPassword(String password) {
        if (ParserUtils.isNullOrEmpty(password)) {
            this.sheetProtectionPassword.unsetPassword();
            this.useSheetProtection = false;
        }
        else {
            this.sheetProtectionPassword.setPassword(password);
            this.sheetProtectionPassword.setPasswordHash(DataUtils.generatePasswordHash(password));
     ...
     * @apiNote Note: This value will be modified to the Excel-internal representation, calculated by
     * {@link DataUtils#getInternalColumnWidth(float, float, float)}}
     */
    ...
    public void addCell(Object value, String address) {
        ...
        addCell(value, adr.getColumn(), adr.getRow());
    }
    ...
    public void addCell(Object value, String address, Style style) {
        Address adr = Cell.resolveCellCoordinate(address);
        addCell(value, adr.getColumn(), adr.getRow(), style);
    }
    ...
    public void addCellFormula(String formula, String address) {
        Address adr = Cell.resolveCellCoordinate(address);
        Cell c = new Cell(formula, Cell.CellType.FORMULA, adr.getColumn(), adr.getRow());
    ...
    public void addCellRange(List<Object> values, String cellRange) {
        ...
        addCellRangeInternal(values, rng.getStartAddress(), rng.getEndAddress(), null);
    }
    ...
    public void addCellRange(List<Object> values, String cellRange, Style style) {
        Range rng = Cell.resolveCellRange(cellRange);
        addCellRangeInternal(values, rng.getStartAddress(), rng.getEndAddress(), style);
    }
    ...
    private <T> void addCellRangeInternal(List<T> values, Address startAddress, Address endAddress, Style style) {
        ...
        int len = values.size();
        for (int i = 0; i < len; i++) {
            list.get(i).setRowNumber(addresses.get(i).getRow());
            list.get(i).setColumnNumber(addresses.get(i).getColumn());
    ...
    public boolean removeCell(String address) {
        Address adr = Cell.resolveCellCoordinate(address);
        return removeCell(adr.getColumn(), adr.getRow());
    }
    ...
    public void setStyle(Range cellRange, Style style) {
        ...
        for (Address address : addresses) {
            ...
            if (this.cells.containsKey(key)) {
                ...
                if (style != null) {
                    addCell(null, address.getColumn(), address.getRow(), style);
                }
            }
        }
    }
    ...
    public void insertRow(int rowNumber, int numberOfNewRows) {
        ...
        var cellsToChange = this.getCells().entrySet().stream().filter(c -> c.getValue().getCellAddress2().getRow() > rowNumber).collect(Collectors.toList());
        ...
        for (var cell : cellsToChange) {
            var row = cell.getValue().getCellAddress2().getRow();
            var col = cell.getValue().getCellAddress2().getColumn();
        ...
        for (Cell cell : upperRow) {
            for (int i = 0; i < numberOfNewRows; i++) {
                Address newAddress = new Address(cell.getCellAddress2().getColumn(), cell.getCellAddress2().getRow() + 1 + i);
    ...
    public void insertColumn(int columnNumber, int numberOfNewColumns) {
        ...
        var cellsToChange = this.getCells().entrySet().stream().filter(c -> c.getValue().getCellAddress2().getColumn() > columnNumber).collect(Collectors.toList());
        ...
        for (Cell cell : leftColumn) {
            for (int i = 0; i < numberOfNewColumns; i++) {
                Address newAddress = new Address(cell.getCellAddress2().getColumn() + 1 + i, cell.getCellAddress2().getRow());
                Cell newCell = new Cell(null, Cell.CellType.EMPTY, newAddress);
                if (cell.getCellStyle() != null)
                    newCell.setStyle(cell.getCellStyle());
                this.getCells().put(newAddress.getAddress(), newCell);
    ...
    public void setCurrentCellAddress(String address) {
        Address adr = Cell.resolveCellCoordinate(address);
        setCurrentCellAddress(adr.getColumn(), adr.getRow());
    }
    ...
    public void setSheetName(String sheetName) {
        if (ParserUtils.isNullOrEmpty(sheetName)) {
    ...
    public void setSplit(Integer numberOfColumnsFromLeft, Integer numberOfRowsFromTop, boolean freeze, Address topLeftCell, WorksheetPane activePane) {
        if (freeze) {
            if (numberOfColumnsFromLeft != null && topLeftCell.getColumn() < numberOfColumnsFromLeft) {
            ...
            if (numberOfRowsFromTop != null && topLeftCell.getRow() < numberOfRowsFromTop) {
    ...
    private void init() {
        ...
        this.sheetProtectionPassword = new LegacyPassword(PasswordType.WORKSHEET_PROTECTION);
    ...
    public String mergeCells(Range cellRange) {
        return mergeCells(cellRange.getStartAddress(), cellRange.getEndAddress());
    }
    ...
    public String mergeCells(String cellRange) {
        ...
        return mergeCells(range.getStartAddress(), range.getEndAddress());
    }
    ...
    public void recalculateAutoFilter() {
        ...
        int start = this.autoFilterRange.getStartAddress().getColumn();
        int end = this.autoFilterRange.getEndAddress().getColumn();
        int endRow = 0;
    ...
    public void resolveMergedCells() {
        ...
        for (Map.Entry<String, Range> range : getMergedCells().entrySet()) {
            pos = 0;
            List<Address> addresses = Cell.getCellRange(range.getValue().getStartAddress(), range.getValue().getEndAddress());
            for (Address address : addresses) {
                Cell cell = null;
                if (!cells.containsKey(address.getAddress())) {
                    cell = new Cell();
                    cell.setDataType(Cell.CellType.EMPTY);
                    cell.setRowNumber(address.getRow());
                    cell.setColumnNumber(address.getColumn());
    ...
    /**
     * Converts a {@link SheetProtectionValue} enum to its XML attribute string representation
     * <p>
     * Java equivalent of {@code internal static string GetSheetProtectionName(SheetProtectionValue)} in the .NET reference.
     * </p>
     *
     * @param protection Sheet protection value to convert
     * @return XML attribute string, or empty string if not matched
     */
    public static String getSheetProtectionName(SheetProtectionValue protection) {
        switch (protection) {
            case objects: return "objects";
            case scenarios: return "scenarios";
            case formatCells: return "formatCells";
            case formatColumns: return "formatColumns";
            case formatRows: return "formatRows";
            case insertColumns: return "insertColumns";
            case insertRows: return "insertRows";
            case insertHyperlinks: return "insertHyperlinks";
            case deleteColumns: return "deleteColumns";
            case deleteRows: return "deleteRows";
            case selectLockedCells: return "selectLockedCells";
            case sort: return "sort";
            case autoFilter: return "autoFilter";
            case pivotTables: return "pivotTables";
            case selectUnlockedCells: return "selectUnlockedCells";
            default: return "";
        }
    }
     ...
     * <p>
     * Java equivalent of {@code internal static WorksheetPane? GetWorksheetPaneEnum(string)} in the .NET reference.
     * </p>
     *
     * @param pane String value to convert
     * @return Matching {@link WorksheetPane} or null if not matched
     */
    public static WorksheetPane getWorksheetPaneEnum(String pane) {
        if (pane == null) {
            return null;
        }
        switch (pane) {
            case "topLeft": return WorksheetPane.topLeft;
            case "topRight": return WorksheetPane.topRight;
            case "bottomLeft": return WorksheetPane.bottomLeft;
            case "bottomRight": return WorksheetPane.bottomRight;
            default: return null;
        }
    }
     ...
     * <p>
     * Java equivalent of {@code internal static SheetViewType GetSheetViewTypeEnum(string)} in the .NET reference.
     * </p>
     *
     * @param viewType String value to convert
     * @return Matching {@link SheetViewType}, defaulting to {@link SheetViewType#normal} if not matched
     */
    public static SheetViewType getSheetViewTypeEnum(String viewType) {
        if (viewType == null) {
        ...
        switch (viewType) {
            ...
            default: return SheetViewType.normal;
        }
    }
