...
public class XlsxWriter {
    ...
    private String createColsString(Worksheet worksheet) {
        if (worksheet.getColumns().size() > 0) {
            ...
            for (Map.Entry<Integer, Column> column : worksheet.getColumns().entrySet()) {
                ...
                float width = DataUtils.getInternalColumnWidth(column.getValue().getWidth());
    ...
    private String createMruColorsString() {
        ...
        for (Color item : this.workbook.getMruColors()) {
            String colorStr = item.getArgbValue();
            if (colorStr == null || colorStr.equals(Fill.DEFAULT_COLOR.getArgbValue())) {
                continue;
            }
            if (!tempColors.contains(colorStr)) {
                tempColors.add(colorStr);
            }
        }
    ...
    private String createRowString(DynamicRow dynamicRow, Worksheet worksheet) {
        ...
        if (worksheet.getRowHeights().containsKey(rowNumber) && Math.abs(worksheet.getRowHeights().get(rowNumber) - worksheet.getDefaultRowHeight()) > FLOAT_THRESHOLD) {
            height = " x14ac:dyDescent=\"0.25\" customHeight=\"1\" ht=\"" + DataUtils.getInternalRowHeight(worksheet.getRowHeights().get(rowNumber)) + "\"";
        }
        ...
        for (Cell item : dynamicRow.getCellDefinitions()) {
            ...
            // Date parsing
                ...
                value = DataUtils.getOADateString(date);
            }
            // Time parsing
                ...
                value = DataUtils.getOATimeString(time);
            }
            else {
    ...
    private String createSheetProtectionString(Worksheet sheet) {
        ...
        if (sheet.getSheetProtectionPassword().passwordIsSet()) {
            sb.append(" password=\"").append(sheet.getSheetProtectionPassword().getPasswordHash()).append('"');
        }
        sb.append("/>");
        return sb.toString();
    }
    ...
    private String createStyleBorderString() {
        ...
        for (Border borderStyle : borderStyles) {
            ...
            if (borderStyle.getLeftStyle() != Border.StyleValue.none) {
                ...
                if (!ParserUtils.isNullOrEmpty(borderStyle.getLeftColor())) {
            ...
            if (borderStyle.getRightStyle() != Border.StyleValue.none) {
                ...
                if (!ParserUtils.isNullOrEmpty(borderStyle.getRightColor())) {
            ...
            if (borderStyle.getTopStyle() != Border.StyleValue.none) {
                ...
                if (!ParserUtils.isNullOrEmpty(borderStyle.getTopColor())) {
            ...
            if (borderStyle.getBottomStyle() != Border.StyleValue.none) {
                ...
                if (!ParserUtils.isNullOrEmpty(borderStyle.getBottomColor())) {
            ...
            if (borderStyle.getDiagonalStyle() != Border.StyleValue.none) {
                ...
                if (!ParserUtils.isNullOrEmpty(borderStyle.getDiagonalColor())) {
    ...
    private String createStyleFillString() {
        ...
        for (Fill fillStyle : fillStyles) {
            ...
            if (fillStyle.getPatternFill() == Fill.PatternValue.solid) {
                sb.append(">");
                if (fillStyle.getForegroundColor().getType() == Color.ColorType.Rgb) {
                    sb.append("<fgColor rgb=\"").append(fillStyle.getForegroundColor().getArgbValue()).append("\"/>");
                }
                Color bg = fillStyle.getBackgroundColor();
                if (bg.getType() == Color.ColorType.Indexed) {
                    sb.append("<bgColor indexed=\"").append(bg.getIndexedColor().getStringValue()).append("\"/>");
                }
                else if (bg.getType() == Color.ColorType.Rgb) {
                    sb.append("<bgColor rgb=\"").append(bg.getArgbValue()).append("\"/>");
                }
                ...
                Color bg = fillStyle.getBackgroundColor();
                if (bg.getType() == Color.ColorType.Rgb) {
                    sb.append("<bgColor rgb=\"").append(bg.getArgbValue()).append("\"/>");
                }
                else if (bg.getType() == Color.ColorType.Indexed) {
                    sb.append("<bgColor indexed=\"").append(bg.getIndexedColor().getStringValue()).append("\"/>");
                }
                sb.append("</patternFill>");
            }
    ...
    private String createStyleFontString() {
        ...
        for (Font fontStyle : fontStyles) {
            ...
            String vertAlignName = Font.getVerticalTextAlignName(fontStyle.getVerticalAlign());
            if (!vertAlignName.isEmpty()) {
                sb.append("<vertAlign val=\"").append(vertAlignName).append("\"/>");
            }
            if (fontStyle.isOutline()) {
                sb.append("<outline/>");
            }
            if (fontStyle.isShadow()) {
                sb.append("<shadow/>");
            }
            if (fontStyle.isCondense()) {
                sb.append("<condense/>");
            }
            if (fontStyle.isExtend()) {
                sb.append("<extend/>");
            }
            sb.append("<sz val=\"");
            sb.append(fontStyle.getSize());
            sb.append("\"/>");
            Color fontColor = fontStyle.getColorValue();
            if (fontColor.getType() == Color.ColorType.Theme) {
                sb.append("<color theme=\"").append(fontColor.getThemeColor().getStringValue()).append("\"");
                if (fontColor.getTint() != null) {
                    sb.append(" tint=\"").append(fontColor.getTint()).append("\"");
                }
                sb.append("/>");
            }
            else if (fontColor.getType() == Color.ColorType.Rgb) {
                sb.append("<color rgb=\"").append(fontColor.getArgbValue()).append("\"/>");
            }
            else if (fontColor.getType() == Color.ColorType.Indexed) {
                sb.append("<color indexed=\"").append(fontColor.getIndexedColor().getStringValue()).append("\"/>");
            }
            else if (fontColor.getType() == Color.ColorType.Auto) {
                sb.append("<color auto=\"1\"/>");
            }
            else {
                // None / default: fall back to theme color 1
                sb.append("<color theme=\"1\"/>");
            }
            sb.append("<name val=\"").append(fontStyle.getName()).append("\"/>");
            sb.append("<family val=\"").append(fontStyle.getFamily().getValue()).append("\"/>");
            ...
            if (fontStyle.getCharset() != Font.CharsetValue.Default && fontStyle.getCharset() != Font.CharsetValue.ApplicationDefined) {
                sb.append("<charset val=\"").append(fontStyle.getCharset().getValue()).append("\"/>");
            }
            sb.append("</font>");
        }
        return sb.toString();
    ...
    private String createStyleNumberFormatString() {
        ...
        for (NumberFormat numberFormatStyle : numberFormatStyles) {
            if (numberFormatStyle.isCustomFormat()) {
                ...
                if (ParserUtils.isNullOrEmpty(numberFormatStyle.getCustomFormatCode())) {
    ...
    private Document createStyleSheetDocument() throws IOException {
        ...
        if (this.workbook.getWorkbookMetadata() != null) {
            if (!ParserUtils.isNullOrEmpty(mruColorString)) {
                sb.append("<colors>");
    ...
    private void createWorkbookProtectionString(StringBuilder sb) {
        if (workbook.isWorkbookProtectionUsed()) {
            ...
            if (workbook.getWorkbookProtectionPassword().passwordIsSet()) {
                sb.append(" workbookPassword=\"");
                sb.append(workbook.getWorkbookProtectionPassword().getPasswordHash());
                sb.append("\"");
            }
            sb.append("/>");
        }
    ...
    private Document createWorksheetPart(Worksheet worksheet) throws IOException {
        ...
        if (!ParserUtils.isNullOrEmpty(colDefinitions)) {
            sb.append("<cols>");
    ...
    private void createSheetViewString(Worksheet worksheet, StringBuilder sb) {
        ...
        for (Map.Entry<Worksheet.SheetViewType, Integer> scaleFactor : worksheet.getZoomFactors().entrySet()) {
            if (scaleFactor.getKey() == worksheet.getViewType()) {
                continue;
            }
    ...
    private void createPaneString(Worksheet worksheet, StringBuilder sb) {
        ...
        if (worksheet.getPaneSplitAddress() != null) {
            ...
            if (worksheet.getPaneSplitLeftWidth() != null) {
                sb.append(" xSplit=\"").append(DataUtils.getInternalPaneSplitWidth(worksheet.getPaneSplitLeftWidth())).append("\"");
                applyXSplit = true;
            }
            if (worksheet.getPaneSplitTopHeight() != null) {
                sb.append(" ySplit=\"").append(DataUtils.getInternalPaneSplitHeight(worksheet.getPaneSplitTopHeight())).append("\"");
                applyYSplit = true;
            }
        }
    ...
    private float calculatePaneHeight(Worksheet worksheet, int numberOfRows) {
        ...
        for (int i = 0; i < numberOfRows; i++) {
            if (worksheet.getRowHeights().containsKey(i)) {
                height += DataUtils.getInternalRowHeight(worksheet.getRowHeights().get(i));
            }
            else {
    ...
    private float calculatePaneWidth(Worksheet worksheet, int numberOfColumns) {
        ...
        for (int i = 0; i < numberOfColumns; i++) {
            if (worksheet.getColumns().containsKey(i)) {
                width += DataUtils.getInternalColumnWidth(worksheet.getColumns().get(i).getWidth());
            }
            else {
